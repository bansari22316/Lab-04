...
def generate_invalid_user_report():
    ...
    data = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}).*(\w+).*SRC=(.*?) ')[1]

    # Generate the CSV report
    df = pd.DataFrame(data)
    csv_filename = f'invalid_users.csv'
    columns = ('Date', 'Time', 'Username', 'IP Address')
    df.to_csv(csv_filename, index=True, header=columns)
...
def generate_source_ip_log(ip_address):
    ...
    Source_IP_ADD = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}.) SRC=(.*?) DST=(.*?) SPT=(.*?) DPT=(.*?) LEN=(.*?) ')[0]


    # Save all records to a plain text .log file
    df = pd.DataFrame(Source_IP_ADD)
